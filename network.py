...
from keras.utils import to_categorical  # Removed np_utils
...
class Corpus:
    ...
    def prepare_data(self, preprocessed=False):
        if not preprocessed:
            ...
            with open(os.path.join(self.corpus_dir, 'rhyme_onehot_{}.pickle'.format(self.language)), 'rb') as jar:
...
class Network:
    ...
    def train_test_split(self):
        self.X_train, self.X_test, y_train, y_test = train_test_split(self.corpus.dataX, self.corpus.labels, test_size=0.2, random_state=42)
    ...
    def build_network_en(self):
        ...
        adam = Adam(learning_rate=self.learning_rate)  # Updated parameter
