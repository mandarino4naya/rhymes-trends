...
class Stanza:
    ...
    def get_char_to_int(self, word):
        int_list = [self.corpus.char_to_int[char] for char in word]
        return int_list

    def pad_to_length(self, word):
        while len(word) < self.corpus.word_length:
            word += ' '
        return word
    ...
    def get_onehot(self, seq):
        seq = [seq]
        onehot = np.array([to_categorical(pad_sequences((data,), self.corpus.seq_length), self.corpus.num_chars + 1) for data in seq])
    ...
    def get_rhyme_scheme(self):
        '''
        Heuristically determines rhyme scheme.
        '''
        rhyme_scheme = {}
    ...
    def get_rhyming_blocks(self):
        '''
        Consolidate the rhyme-coded text into blocks.
        '''
        ...
        def in_ending(word, line_endings):
            return any(word in ending and '\n' in ending for ending in line_endings)

        def combine_code(group_list, code):
            combined_words = ' '.join([item[0] for item in group_list])
            return combined_words, code
        ...
        for key, group in itertools.groupby(coded_words, lambda x: x[1]):  # Rhyme code
            ...
            if len(group_list) == 1:
                ...
                if not ending_found:
                    combined_words = ' '.join([item[0] for item in group_list])
                    rhyme_blocks.append((combined_words, key))

        return rhyme_blocks

    def scheme_to_orig(self, rhyme_blocks):
        '''
        Back-maps the rhyme scheme to the original text.
        '''
...
def Poem_func(language, input_file, output_path):
    if language == 'english':
    ...
    output_filename = os.path.basename(input_file).split('.')[0] + '_r.' + os.path.basename(input_file).split('.')[1]
